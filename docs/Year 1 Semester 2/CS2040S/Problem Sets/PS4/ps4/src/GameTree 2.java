...
public class GameTree {
    ...
    public int findValue(TreeNode node, int height) {
        // Base case: leaf node.
        if (node.numChildren == 0){
            // Since leaf node is already initialized.
            return node.value;
        }
        // Recursive case.
        int best = findValue(node.children[0],height+1);
        // Find the child that leads to the leaf with the best value.
        for (TreeNode t: node.children){
            // If the child node is player 2, player 2 will choose
            // the worst case for player 1.
            int temp = findValue(t,height+1);
            if (height%2 != 0){ // This means node is player 2.
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(tree.findVarValue(tree.root,0));
